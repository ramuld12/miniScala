...
object Lambda {
  ...
  def main(args: Array[String]): Unit = {
    ...
    assert(decodeNumber(eval(encode(IntLit(5)),Map[Id, Val]())) == 5)
    assert(!decodeBoolean(eval(encode(Parser.parse("0 + 1 == 0")),Map[Id, Val]()))) //Should return false
    assert(decodeBoolean(eval(encode(Parser.parse("1 - 1 == 0")),Map[Id, Val]()))) //Should return true
  ...
  def encode(e: Exp): Exp =
    e match {
      case IntLit(c) if c >= 0 => // non-negative integer literals are encoded as on slide 18
        def encodeIntLit(c: Int): Exp = //Help function to produce 's' recursively
          if (c == 0) VarExp("z")
          else CallExp(VarExp("s"), List(encodeIntLit(c-1)))
        LambdaExp(List(FunParam("s",None)), LambdaExp(List(FunParam("z",None)), encodeIntLit(c)))
      ...
      case BinOpExp(leftexp, EqualBinOp(), IntLit(0)) => // e == 0, slide 18
        CallExp(CallExp(encode(leftexp), List(LambdaExp(List(FunParam("n", None)), encode(BoolLit(false))))),
        List(encode(BoolLit(true))))
      ...
      case UnOpExp(op, subexp) =>
        op match {
          case NotUnOp() => // !e, slide 15
            encode(IfThenElseExp(subexp, encode(BoolLit(false)), encode(BoolLit(true))))
          case _ => // remaining cases are not (yet) implemented
            throw new EncoderError(e)
        }
